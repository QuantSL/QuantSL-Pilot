...
function generate_n_test(; parameters::Dict)
	...
	os1() = OpSum() + Op("Sx", indexDict[(:a, i)]) * Op("Sx", indexDict[(:b, i)])
	return (i) -> OpSum() + os1()
end
...
function generate_n(; parameters::Dict)
	...
	os1(i) = OpSum() + Op("Sz", indexDict[i]) * Op("Sz", indexDict[j])
	return (i, j) -> OpSum() + os1(i)
end
...
function generate_H_int(; parameters::Dict)
	...
	os1(i, j) = OpSum() + parameters[:U_int] * 
	os2(i, j) = OpSum() + sum(i -> 
		sum(j -> 
			os2_1(i, j), [1:parameters[:N];]
		), [1:parameters[:N];]
	)
	return OpSum() + os1(i, j) * os2(i, j)
end
...
function generate_H_hopping(; parameters::Dict)
	...
	indexDict = Dict(key => val for (val, key) in enumerate(indices))
		...
		os2_1(i, j) = OpSum() + sum(j -> 
			os2_1_1(i, j), [1:parameters[:N];]
		)
	os1(i, j) = OpSum() + parameters[:J] * 
	os2(i, j) = OpSum() + sum(i -> 
		os2_1(i, j), [1:parameters[:N];]
	)
	return OpSum() + os1(i, j) * os2(i, j)
...
function generate_H_lattice(; parameters::Dict)
	...
	n_test = _generate_n_test(; indexDict = indexDict, parameters = parameters)
		...
		os1_1(i, j) = OpSum() + n_test(i) * n_test(i) * Op("Sz", indexDict[i])
	os1(i, j) = OpSum() + sum(i -> 
		os1_1(i, j), [1:parameters[:N];]
	)
	return OpSum() + os1(i, j)
end
...
function generate_H(; parameters::Dict)
	...
	os1(i, j) = OpSum() + H_int + H_hopping + H_lattice
	return OpSum() + os1(i, j)
end
...
function generateSystem(; parameters::Dict)
	...
	OperatorContainer(
		_generate_n_test(basis = basis, indexDict = indexDict, parameters = parameters), 
		_generate_n(basis = basis, indexDict = indexDict, parameters = parameters), 
		_generate_H_int(basis = basis, indexDict = indexDict, parameters = parameters), 
		_generate_H_hopping(basis = basis, indexDict = indexDict, parameters = parameters), 
		_generate_H_lattice(basis = basis, indexDict = indexDict, parameters = parameters), 
		_generate_H(basis = basis, indexDict = indexDict, parameters = parameters)
	)
end
